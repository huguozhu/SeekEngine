...
uint64_t D3D11RHIFence::Signal()
{
	...
	uint64_t const id = m_iFenceVal;
	++m_iFenceVal;

	d3d_imm_ctx->End(query.Get());

	m_mFences[id] = std::move(query);

	return id;
}
void D3D11RHIFence::Wait(uint64_t id)
{
	auto iter = m_mFences.find(id);
...
bool D3D11RHIFence::IsCompleted(uint64_t id)
{
	auto iter = m_mFences.find(id);
	if (iter == m_mFences.end())
	{
