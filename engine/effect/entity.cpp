...
#define DVF_MACRO_FILE_UID 21     // this code is auto generated, don't touch it!!!
...
DVFResult Entity::Tick(float delta_time)
{
    for (ComponentPtr component : m_vOwnedComponents)
    {
        DVF_RETIF_FAIL(component->Tick(delta_time));
    }
    return DVF_Success;
}

DVFResult Entity::AddToTopScene()
{
    return AddToParent(m_pContext->SceneManagerInstance().GetRootComponent().get());
}

DVFResult Entity::DeleteFromTopScene()
{
    m_pContext->SceneManagerInstance().GetRootComponent()->DelChild(m_pRootComponent);
    return DVF_Success;
}

DVFResult Entity::AddToParent(Entity* parent_entity)
{
...
DVFResult Entity::AddToParent(SceneComponent* parent_component)
{
